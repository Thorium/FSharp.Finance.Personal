...
module InvoiceFactoring =
    ...
    module FactoringParameters =
        ...
        let build (advances: InvoiceAdvance array) (interestConfig: Interest.BasicConfig option) : Parameters =
            ...
            let startDate = advances |> Array.map (fun a -> a.Invoice.IssueDate) |> Array.min

            // Calculate total principal as sum of net advances
            let principal = advances |> Array.sumBy (_.NetAdvance)
            ...
            let scheduledPaymentMap =
                ...
                |> Array.map (fun advance ->
                    ...
                    let scheduledPayment =
                        ScheduledPayment.quick (ValueSome advance.Invoice.FaceValue) ValueNone
            ...
            let basicParams = {
                ...
                InterestConfig =
                    ...
                    |> fun defaultConfig -> interestConfig |> Option.defaultValue defaultConfig
            }
        ...
        let calculateStatistics (advances: InvoiceAdvance array) =
            if Array.isEmpty advances then
                ...
                let weightedAvgAdvanceRate =
                    if totalFaceValueDecimal = 0m then
                        0m
                    else
                        (advances
                         |> Array.sumBy (fun a -> Cent.toDecimal a.Invoice.FaceValue * a.AdvanceRate))
                        / totalFaceValueDecimal
                ...
                let weightedAvgFeeRate =
                    if totalFaceValueDecimal = 0m then
                        ...
                        (advances
                         |> Array.sumBy (fun a -> Cent.toDecimal a.Invoice.FaceValue * a.FeeRate))
                        / totalFaceValueDecimal
